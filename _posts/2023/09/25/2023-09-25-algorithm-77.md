...
function solution(id_list, report, k) {
    ...
    const countObj = Object.values(convertReport).reduce(
        (acc, cur) => {
...
function solution(id_list, report, k) {
    ...
    const countObj = reports.reduce(
        (acc, cur) => ((acc[cur[1]] = acc[cur[1]] + 1 || 1), acc),
        {}
    );
...
> [문제 출처 - 알고리즘](https://school.programmers.co.kr/learn/courses/30/lessons/92334)
